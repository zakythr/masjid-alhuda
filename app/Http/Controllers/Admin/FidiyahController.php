...
class FidiyahController extends Controller
{
    private $folder = 'admin.fidiyah';
    private $uri = 'admin.fidiyah';
    private $title = 'Data Fidiyah';
    private $desc = 'Description';

    use CrudTrait;

    public function __construct(Fidiyah $table) 
    {
    	$this->table = $table;
    }
    ...
    public function filter(Request $request)
    {
        ...
        if (!empty($request->tanggalfidiyah)) {
            $request->session()->put('tanggalfidiyah', $request->tanggalfidiyah);
        } else {
            $request->session()->forget('tanggalfidiyah');
        }
    ...
    {
    	if ($request->ajax()) {
    		$data = $this->table->select(['id', 'nama','sex_id','jmlhari','total','tanggalfidiyah','alamat','created_at'])->orderBy('id')->orderBy('id', 'desc');
            if ($request->session()->has('sex_id')) {
                $data = $data->where('sex_id', $request->session()->get('sex_id'));
            }
            if ($request->session()->has('tanggalfidiyah')) {
                $data = $data->where('tanggalfidiyah', $request->session()->get('tanggalfidiyah'));
            }
            ...
            // ->editColumn('id','<input type="checkbox" class="checkbox" name="id[]" value="{{$id}}"/>')
                ...
                ->editColumn('tanggalfidiyah', function ($index) {
                    return date('d/m/Y', strtotime($index->tanggalfidiyah));
                })
    ...
    public function show(FormBuilder $formBuilder, $id)
    {
        ...
        $data['form'] = $formBuilder->create('App\Forms\FidiyahForm', [
            'method' => 'PUT',
            'model' => $tbl,
